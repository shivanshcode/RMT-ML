...
## Supercollapse MLP sweep

A second workload, separate from the Llama analysis: the same `rmt` suite applied to the weight
matrices of **supercollapse MLP checkpoints**, under muP and a no-muP ablation, across eight widths
and three seeds. Nothing in [Analysis](#analysis-benchmark-and-test-results) covers it — that
section is the Llama-3.1-8B run. These checkpoints have been processed but not yet scored against
matched-shape null bands.

| File | Role |
|---|---|
| `rmt_supercollapse.py` | Wraps the training side's `.npz` checkpoints in an `nn.Module` the suite can discover, and drives the analysis. Also a library and a CLI. |
| `run_rmt_supercollapse.slurm` | 16-task array: 8 widths × {muP, ablation}, 36 (seed, fraction) units per task, one per CPU. |
| `make_plot_pdfs.py` | Contact sheets at **fixed width**, tiling the training fractions. |
| `make_width_pdfs.py` | Contact sheets at **fixed fraction**, tiling the widths — the comparison supercollapse is about. |

**Grid.** Widths 384, 512, 645, 812, 1024, 1290, 1625, 2048; `N = 5` blocks; seeds 0, 1, 2;
fractions 0.00, 0.05, 0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 1.00. That is
16 × 36 = **576 runs**, and all 576 carry a `done` marker.

**Discovery.** The config advertises `model_type = "supercollapse_mlp"`, which matches none of the
registry keys, so `rmt.discovery.get_model_spec` falls through to the generic spec
(`rmt/discovery.py:123`) — the one that maps `fc1 -> "U"`, `fc2 -> "D"` and reads the layer index
from `\.layers?\.(\d+)\.`. Ten matrices per checkpoint (5 layers × 2 projections), analysed by
the ordinary `analyze_one_model` path, so every row is the same 133-column schema as the Llama CSV.

### Two traps this driver exists to avoid

Both are properties of the suite, not of the checkpoints, and both silently corrupt the result
rather than raising.

1. **Zero matrices.** `fc2` is zero-initialised, so at `frac0p00` every `D` matrix is exactly 0.
   Then σ = 0, the MP support degenerates, `scipy.optimize.brentq` raises inside `rmt.mp.mp_median`
   — and the pipeline catches it and drops the row. The output is a CSV missing half its matrices
   with nothing to say so. `load_npz_model` drops all-zero matrices up front instead and records
   them on `model.rmt_dropped`. This is directly visible in the committed tables: the D = 1024
   roll-up has **345 rows, not 360**, because the 15 `D` matrices at `frac0p00` (3 seeds × 5 layers)
   are absent by design — `U` appears 180 times (12 fractions) and `D` 165 (11).
2. **Averaging U with D.** `rmt.pipeline.analyze_checkpoints` averages every record in a layer, so a
   layer's stable rank is `mean(fc1, fc2)`. At frac 0 that is `mean(fc1, 0)` — half the true `fc1`
   value, an artefact masquerading as a training trend. `track_scalars()` replaces it with a tidy
   long-format table keyed by `(frac, layer, short)`, so the two roles never mix, and reports a zero
   matrix as zero rather than dragging a layer mean down.

**Estimator settings, and how they differ from the Llama run.** `rmt_supercollapse.py` overrides
only the block switches (`_PIPELINE_FLAGS`), so everything numerical is the `RunConfig` default.
Read back from the committed CSVs, the sweep ran at **unfolding degree 7** with
**`spacing_bulk_mode = "center"`, `bulk_center_frac = 0.7`** — 718 of 1024 levels kept at D = 1024 —
and `sigma2_n_windows = 200000`. The Llama run used degree 15 and `--spacing_bulk_mode auto`
instead, so the two are *not* the same estimator and their numbers should not be compared directly;
see [§A.1](#a1-what-was-tested-with-what-and-at-which-settings) for why the degree matters. The
unfolding branch is also not homogeneous here: 344 of the 345 D = 1024 rows took `cheb` and one took
`gauss`, whose Σ² is only calibrated to L ≤ 5 (`rmt/spacing.py:56`).

Delta models `W(t) − W(0)` are built in memory at float64 (`load_delta_model`), not by writing a
temporary `.npz` next to the checkpoints and subtracting in stored fp32. For `fc2` the delta equals
the raw weight, since `W(0) = 0`, so delta analysis is only distinct for `fc1`.

### Running it

```bash
# one width, one parameterization, from the login node
python rmt_supercollapse.py --ckpt-dir ~/supercollapse/ckpts \
    --D 1024 --seeds 0 1 2 --out ./RMT_Out_D1024

python rmt_supercollapse.py --ckpt-dir ... --D 1024 --list          # dry run
python rmt_supercollapse.py --ckpt-dir ... --D 1024 --scalars-only  # no plots, no 133-col CSV
python rmt_supercollapse.py --ckpt-dir ... --D 1024 --delta         # W(t) - W(0)
...
# contact sheets
python make_plot_pdfs.py  --D 1024                 # fractions at fixed width
python make_width_pdfs.py --frac 1.00              # widths at fixed fraction
python make_width_pdfs.py --frac 1.00 --nomup      # the ablation trees
```

`--backend numpy` is pinned in the SLURM script on purpose. The `auto` backend switches to the GPU
at `max(n, m) >= 1024` (`rmt/config.py:54`), which would put D < 1024 on numpy and D >= 1024 on
cuSOLVER — a backend change in the middle of the width sweep that is the independent variable.

### Where the output lands

The scripts write `RMT_Out_D<D>/` and `nomup_RMT_Out_D<D>/` **into `$OUT_ROOT`, which the SLURM
script sets to the repository root** (`run_rmt_supercollapse.slurm:30`). The committed results were
afterwards restructured one level down, into `shree-supercollapse-rmt-results/`:

```
shree-supercollapse-rmt-results/
    RMT_Out_D384/ … RMT_Out_D2048/              muP, 8 widths
        mlp_D1024_N5_seed0_frac0p50/            36 per width: 3 seeds x 12 fractions
            esd/ hill/ spacing/ rigidity/ porter_thomas/ porter_thomas_decile/
            mlp_D1024_N5_seed0_frac0p50_matrix_metrics.csv     10 rows (5 at frac 0)
            mlp_D1024_N5_seed0_frac0p50_summary.json/.png
            unit.log   done
        pdfs/                                   6 PDFs from make_plot_pdfs.py
    nomup_RMT_Out_D384/ … nomup_RMT_Out_D2048/  the no-muP ablation
    pdfs_by_width/  pdfs_by_width_no_mup/       30 PDFs each, from make_width_pdfs.py
```

2.8 GB over 37,312 files, so it is **git-ignored and excluded from the knowledge graph**
(`.gitignore`; graphify honours it). Three consequences for anyone re-running:

- The scripts reproduce the *flat* layout at the repo root, not the nested one above. Point
  `make_plot_pdfs.py --root` and `make_width_pdfs.py --base` at wherever the trees actually are.
- **There is no combined table across the grid.** `analyze_width` writes a per-width roll-up at the
  top of `--out`, but the SLURM script gives each unit a private `mktemp` directory (so 36 workers
  do not race on one file) and then moves `$TMP/*.csv` into the *run* directory. The roll-up
  therefore lands inside each run folder containing that one unit's rows — a duplicate of the
  per-run CSV, not a roll-up. Genuine width-level tables (345 rows) exist only for `RMT_Out_D384`
  and `RMT_Out_D1024`, built separately afterwards. To analyse the sweep, concatenate the per-run
  `*_matrix_metrics.csv` files yourself.
- The PDFs are 96 per-width sheets (16 roots × 6 plot kinds) and 60 across-width sheets (2 modes ×
  6 kinds × fractions 0.40, 0.50, 0.60, 0.80, 1.00). The across-width sheets do not cover all 12
  fractions.
...
| `shree-supercollapse-rmt-results/` | `run_rmt_supercollapse.slurm`, 2026-09-17 — **untracked**, see [Supercollapse MLP sweep](#supercollapse-mlp-sweep) | 576 runs | 7 | `center` @ 0.7 |
