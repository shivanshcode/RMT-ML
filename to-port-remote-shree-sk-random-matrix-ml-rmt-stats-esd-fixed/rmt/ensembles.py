...
def ginoe(n, rng=None) -> np.ndarray:
    """Ginibre Orthogonal Ensemble: **real** iid n×n, entries N(0, 1/n).

    The correct Ginibre reference for a real matrix (an LLM weight matrix is
    real).  Unlike GinUE its spectrum contains an O(√n) subset of exactly real
    eigenvalues, so ⟨cos arg z⟩ = −0.175, not the GinUE −0.216
    (``rmt.spacing.CSR_REFERENCE``).
    """
    g = _rng(rng)
    return g.standard_normal((n, n)) / np.sqrt(n)
