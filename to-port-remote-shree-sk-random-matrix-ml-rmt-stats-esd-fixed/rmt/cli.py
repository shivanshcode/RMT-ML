...
_CHOICES = {
    ...
    # gauss = adaptive Gaussian broadening (Thamm); poly = legacy global fit
    "unfold_method": ["auto", "cheb", "gauss", "poly"],
    "spacing_domain": ["sval", "eig"],
    # center = keep a centred fraction by rank (review §4.1); mp = legacy
    # [nu_-, nu_+] trim, which is a no-op on square matrices.
    "spacing_bulk_mode": ["auto", "center", "mp", "none"],
}
...
def main(argv: Optional[list] = None) -> int:
    ...
    if args.selftest:
        ...
        # verbose=True so the SLURM gate actually logs which checks ran; the
        # exit code alone gives no evidence in the job log that it did anything.
        ok = run(seed=args.seed or 1234, verbose=True)
