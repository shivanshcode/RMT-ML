...
- The static-audit pass makes decile prevalidation metadata-only and physical-parameter-bounded; enforces strictness after optional stages; validates cache ordering/actual dtypes; qualifies weight singular subspaces; aligns component-aware discovery; makes activation replay artifacts/failures transactional; honors disabled overlap; requires exact analyzed lesion scope; propagates spacing seeds; serializes random Hill support; rejects degenerate modified-MP fits and unsupported library config; materializes checkpoint iterables; structures optional power-law adapter status; and consolidates synthetic fixtures. See `bug_report.md` for status.
