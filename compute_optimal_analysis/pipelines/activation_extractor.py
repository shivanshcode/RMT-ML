...
class CovarianceAccumulator:
    ...
    def update(self, activations: Tensor, *, max_samples: int | None = None) -> None:
        ...
        flattened = activations.detach().reshape(-1, self.dimension).to(
...
class ActivationExtractor:
    ...
    def _update(self, key: str, tensor: Tensor) -> None:
        ...
        accumulator.update(tensor, max_samples=self.max_samples_per_hook)
...
def compute_tensor_svd(
    ...
    *,
    ...
    analysis = matrix.detach().to(device=target, dtype=torch.float32)
    ...
    return SVDResult(
        U=U.float().cpu().numpy(),
        s=singular_values.float().cpu().numpy(),
        Vh=Vh.float().cpu().numpy(),
