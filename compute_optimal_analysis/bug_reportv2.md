...
All confirmed v2 findings below have been repaired in the current working tree. Each item is marked **DONE** after its implementation/documentation update. The local regression suite now reports **79 passed**; CUDA/HPC execution still requires the documented deployment validation.
...
### ✅ DONE — CO-V2-01 — High: the factor Lanczos adapter now replaces the Lanczos support with an analytic MP law
...
### ✅ DONE — CO-V2-02 — High: partial batches and skipped updates still terminate training before the planned token budget
...
### ✅ DONE — CO-V2-03 — High: “measured training FLOPs” still count loss targets rather than executed work
...
### ✅ DONE — CO-V2-04 — High: the IsoFLOP repair can collapse all allocation regimes into the same experiment while retaining different labels
...
### ✅ DONE — CO-V2-05 — High: spectrum-domain consistency is only partially repaired
...
### ✅ DONE — CO-V2-06 — Medium: the zero-matrix shortcut crashes the default runner; low-rank/short-recurrence failures still abort a cell
...
### ✅ DONE — CO-V2-07 — Medium: positive ridge is removed from reported edges/poles but not from the Stieltjes transform
...
### ✅ DONE — CO-V2-08 — Medium: several production tracks still repeat CPU SVDs despite the one-SVD contract
...
### ✅ DONE — CO-V2-09 — High for small-SV studies: production analysis still cannot request float64 SVD independently of model dtype
...
### ✅ DONE — CO-V2-10 — Medium: count-matched “bulk” lesions now silently include top/bottom directions
...
### ✅ DONE — CO-V2-11 — Medium: energy matching remains scientifically unmatched, and the new status is discarded
...
### ✅ DONE — CO-V2-12 — Medium: spacing degeneracies are still deleted in the runner, and retained zero gaps use the wrong mean-one normalization
...
### ✅ DONE — CO-V2-13 — Medium: the Brody boundary standard-error fix does not actually exclude asymmetric stencils
...
### ✅ DONE — CO-V2-14 — Medium: zero eigenvalues remain excluded from KDE summaries and analytic MP KS
...
### ✅ DONE — CO-V2-15 — Medium: fit-status persistence is incomplete, and upper-outlier counts still have inconsistent definitions
...
### ✅ DONE — CO-V2-16 — Medium: paper mode names still do not select their scientific method presets
...
### ✅ DONE — CO-V2-17 — Medium: plot execution and Brody labels do not honor the resolved diagnostic settings
...
### ✅ DONE — CO-V2-18 — High: direct runner invocations can mix new manifests with stale results
...
### ✅ DONE — CO-V2-19 — Medium: partial asset staging can now silently re-certify modified old assets with stale provenance
...
### ✅ DONE — CO-V2-20 — Medium: asset manifests are not portable from Windows staging to Linux execution
...
### ✅ DONE — CO-V2-21 — Medium: the library's windowed-Hill cutoff/count metadata is fabricated from plateau width
...
### ✅ DONE — CO-V2-22 — Medium/conditional: the incompatible top-level `rmt` namespaces remain
...
### ✅ DONE — CO-V2-23 — Low: migration documentation still asserts a NumPy incompatibility that the sibling fix removed
...
All confirmed v2 findings are repaired and marked above. The fixes include authoritative recycled-loader token stopping, separate physical-work accounting, collapsed-allocation rejection/labeling, one-domain spectral plumbing, structured unavailable matrix results, consistent ridge resolvents, factor reuse, independent float64 analysis precision, explicit lesion feasibility/matching records, complete-mean zero-gap spacing, complete-ESD MP summaries, full fit diagnostics, scientific mode presets, plot gating, fresh runner directories, and portable/verified partial asset staging.

Remaining broader design/scientific limitations (not reopened v2 bugs):
...
1. Run the complete suite from this directory in its own process (current local result: 79 passed).
2. Verify staged assets and launcher LF bytes, then run a tiny CPU/GPU calibration with a fresh output directory.
3. Inspect realized allocation ratios, token/work counters, fit availability/diagnostics, and lesion match records before a costly allocation.
4. Validate one small job against the actual cluster CUDA/BF16/compiler stack before production.
