...
This audit compares Phase I with each source-bearing file in `codebase1`, `codebase2`, and `codebase3`. It compares the 2025 methods with supplied papers, `farmscode`, and `lanczoscode`. Data arrays, serialized checkpoints, generated figures, and notebook outputs were inventoried but were not interpreted as executable specifications. Instructions embedded in notebooks, comments, papers, or archived command examples are reference material only. The controlling instructions are the user task and the repository contracts.
...
- `codebase2` is the most detailed level-statistics reference. It supplies Gaussian broadening, Gaussian-CDF unfolding, Wigner KS, number variance, Brody CDF, Porter-Thomas, IPR, Hill, and CSN bootstrap.
...
- Phase I already corrects several reference behaviors. It uses one reduced-SVD container, explicit normalization, bounded Brody MLE, deterministic number variance, sign-invariant overlap, centered covariance, and normalized entropy.

Four statements in the Phase II task require correction:

1. FARMS is not a pointwise eigenvalue map. Hu et al. define Fixed-Aspect-Ratio Matrix Subsampling. It samples equal-shape submatrices at one ratio. It computes and concatenates their spectra, which averages their ESDs. The heavy-tail fit uses this pooled spectrum.
2. The Lanczos paper does not use only a finite tridiagonal resolvent for asymptotic support. It applies Lanczos and factors the Jacobi matrix with Cholesky. It estimates limiting Cholesky entries and extends the factor with a semi-infinite constant tail. A continued fraction gives the Stieltjes transform.
...
The codebase3 symbol `Q` is generally `d/k >= 1`, so its formulas use `1/Q=q`. The released FARMS code defines `Q_ratio` operationally as sampled columns divided by sampled rows and does not bound it by one. APIs thus use `target_aspect_ratio` for that FARMS target and `aspect_ratio` for canonical `q=min(shape)/max(shape)`.
...
| MP scale | Resistant MP quantile matching and iterative upper-spike removal | Gavish-Donoho median of squared singular values matched to the numerical MP median | Modified singular MP curve with free amplitude and upper edge | Edge from a retained maximum and a KDE residual for `sigma` | Expose analytic, KDE, FARMS, and Lanczos methods |
| MP support | Analytic support and Johnstone edge scale | Analytic singular support with median scale | Empirical lower edge plus fitted upper edge is not constrained to one MP aspect ratio | Analytic support using reciprocal aspect notation | Keep each result fitted assumptions explicit. Never label the unconstrained codebase2 curve as an analytic MP fit |
...
| Brody fit | Bounded `[0,1]` spacing-likelihood MLE | Not implemented | Nonlinear least-squares fit to empirical spacing CDF, bootstrapped. Its historical bound permits beta more than one | Not implemented | Keep bounded MLE default and add bounded CDF-NLS alternative with optional bootstrap |
...
The next tables classify the full repository inventory. Path families cover every file under `farmscode/`.

| Path or full path family | Audit classification |
|---|---|
...
The released code shows that FARMS is Fixed-Aspect-Ratio Matrix Subsampling. It is not a pointwise map of eigenvalues. For a matrix with `rows` and `cols`, the linear-layer samplers define
...
The `fixed_sampling` path computes one floor-divided stride per dimension. It uses the requested operation count and limits starts. It does not force a missed final endpoint. The `matrix_size_dependent_sampling` path uses one fixed stride. The reference preserves source orientation. Its convolution path slices channel matrices separately and applies the historical convolution scaling before SVD.

#### Discrepancies found

Phase II defined `target_aspect_ratio` as rows divided by columns. It used the shorter dimension for `window_size` and rounded grid starts. It transposed wide input by default. Those choices preserve a fixed shape but do not reproduce the released sampler for non-square targets.

The released repository also has copy-to-copy edge-case differences. One sampler clamps `num_row_samples` but can still request too many columns. Another omits that clamp. One LLM copy leaves its convolution dispatch unavailable. Silent short slices can thus produce unequal window shapes. These behaviors are implementation defects rather than scientific requirements and are not copied into the pure engine.

#### Mathematical and numerical fixes applied

- `target_aspect_ratio` means sampled columns divided by sampled rows.
- `window_size` means sampled rows, matching `num_row_samples`.
...
- Every requested window must fit and all pooled windows have identical dimensions, eliminating the reference silent short-slice failure.
...
- The standardized project container stores the pooled values descending, unlike the reference ascending work array. This permutation leaves the ESD and all order-aware fits unchanged and preserves the repository-wide singular-spectrum convention.
...
For a fixed window shape, multiplying every eigenvalue by one constant does not change a CSN or Hill tail exponent. Canonical normalization is thus reference-equivalent for tail shape while making MP support values comparable across window sizes.
...
`LanczosTri` and `AsympLanczos` draw and normalize Gaussian probes. This gives uniform probes on the real unit sphere. Production uses two full reorthogonalization passes. The adaptive routine compares local means and sample deviations in consecutive windows. The reference uses an absolute tolerance relative to trace. The maintained sequence length uses `floor(log(N)/2)`, and the test interval is two. Maximum iterations include `6 log(N)+24`, `N/4`, and `sqrt(N)`. Recurrence, convergence, and Cholesky-pivot defaults use operator-relative scales. Thus, factor scale cannot change availability.

`CholeskyList` first retains the true finite Jacobi Cholesky factor for spike estimation. It averages the stable suffix in Jacobi coordinates. It adds two constant diagonal entries and one constant off-diagonal entry. Then it factors the changed matrix and averages terminal Cholesky coefficients across probes. If those limiting bidiagonal coefficients are `a` and `b`, the one-cut support is
...
`BiRef` supplies the constant-tail Stieltjes solution and `BiRel` propagates it through each finite prefix. `EstimDensity` averages the resulting imaginary parts across probes. The reference `EstimSpike` uses the absolute rule `gamma_plus + c N^-delta`. The production adapter deliberately uses the scale-relative rule `gamma_plus + c gamma_plus N^-delta` so equivalent spectra in transformer-scale units receive equivalent decisions. It takes the modal count across probes and reports locations from a qualifying recurrence. The released detector has no separate fixed-point support equation and no residue threshold. Ritz residues are squared first components of tridiagonal eigenvectors. They are the VEST pole weights.

#### Discrepancies found

Phase II used a fixed recurrence length and directly averaged a terminal Cholesky window. It used one representative prefix for density. An extended constant-tail finite section gave poles. These are defensible extensions, but they differ from the released implementation adaptive Jacobi stabilization, modified-tail ordering, multi-probe density average, and finite-Ritz spike rule.

The released `EstimSpike` gets its threshold edge from the first finite Cholesky tail, not consensus support. Its threshold-return branch refers to a variable with different capitalization. The production engine uses consensus support and a specified threshold. This removes probe-order dependence and corrects the unreachable return defect.

#### Mathematical and numerical fixes applied

- The default maximum recurrence includes the reference `N/4` term.
...
- Ritz residues are always reported and can be filtered by an optional nonnegative threshold. A zero threshold reproduces reference counting.
- The production spike threshold uses the consensus scale-relative `lambda_plus + c lambda_plus N^-delta`, not the first probe noisy terminal coefficients. Output records the effective margin and convention.
- Rectangular factors use matrix-free covariance products and exact trace scaling without materializing a dense covariance.
- Diagnostics report per-probe recurrence lengths, convergence flags, edges, pole counts, residues, consensus coefficients, and selected pole method.
...
The supplied example identifies one GPU, 64 GiB, and the `gpulong` partition. It sets offline controls, disables tokenizer parallelism, and uses expandable CUDA segments. It records `/home/shivansh/.conda/envs/rmt_ml_env/bin/python` as a successful interpreter. It does not establish exact package/Python versions, a successful CUDA module load, or the availability of a versioned CUDA/Python module. Its Pythia snapshot, raw WikiText path, legacy `python -m rmt` controls, and ESD path belong to the sibling application. This training runner does not use them.

`run_hpc.slurm` calls the recorded interpreter, with an optional `RMT_PYTHON` override. It declares `gpulong`. It does not load or purge modules or activate a guessed `.venv`. An operator can set `RMT_CUDA_MODULE` only after a successful test with the live Torch stack. The launcher gets this directory from `PROJECT_ROOT` or `SLURM_SUBMIT_DIR`. `PYTHONPATH` contains only this source. The launcher examines local `rmt.__file__`, CUDA, BF16, and assets. It uses writable compiler caches and a new output root. It invokes only `run_experiments.py` flags defined by `pipelines/cli_config.py` and exposes `TRACK=paper1|paper2|paper3|golden|all`. The one-process runner still requests one GPU and does not imply unimplemented distributed scaling. Package changes use an environment inventory. Unpinned ESD requirements cannot give an exact cluster lock. Their NumPy API requirements conflict with standalone NumPy 1.26.4.

The Phase III prose calls the Golden synthesis "Chebyshev Unfolding," while its normative Track 4 command and exhaustive CLI table select `spline_monotone`. The executable track follows the explicit command and table. Chebyshev remains the Track 2 reproduction strategy and is available with one flag change for a Golden sensitivity study.

## 5. Detailed reconciliation findings

### 5.1 `codebase1`: dual-end and lesion reference

The activation hook computes a cumulative mean in its first pass. Its second pass computes the cumulative average of batch Gram matrices. It subtracts the first-pass mean, which establishes that the relevant feature matrix is centered. It does not divide each Gram matrix by its number of flattened tokens. Thus, batches have equal weight, and scale depends on batch size. Phase I accumulator instead adds sample sums and Gram matrices and divides once by the total sample count.

The paper-figure notebook defines a singular-value MP density. It matches the median of squared singular values to a numerical MP median. It can first remove a leading fraction. This is aligned with Phase I median scale estimator after converting domains.

The reference overlap routine sorts activation eigenvectors in descending order. It computes a reduced SVD and multiplies `Vh` by activation eigenvectors. Then it gets the maximum in each row. It does not take an absolute value before the maximum and does not square the result. Consequently, sign choices can change the plotted statistic. The repository squared overlap is invariant to singular/eigenvector signs and matches the paper projection interpretation more closely.
...
`GaussBroadening` gets a local width from neighbors `winSize` ranks away. It broadens the spectrum with Gaussian sums. Their summed CDF unfolds the spectrum. Boundary handling either repeats edge levels or drops boundary levels. This is the actual codebase2 unfolding method.

Its modified singular-value MP curve has free amplitude, upper edge, and lower edge. The fitting routine fixes the lower edge to an observed order statistic. It restricts the fit near peak density. Nonlinear least squares fits amplitude and upper edge. This is an empirical curve fit, not a one-parameter MP variance estimate.

Brody fitting operates on the empirical spacing CDF, uses nonlinear least squares, and bootstraps spacing samples to estimate error. The historical implementation allows beta more than one. Spectral-Chinchilla bounds both MLE and CDF-NLS variants to `[0,1]`, because the CLI contract defines the Poisson-to-GOE interpolation.

Number variance samples random interval centers until a rolling set of estimates converges or a maximum iteration count is reached. This differs from Phase I deterministic evenly spaced windows. Both are retained.
...
The power-law path fits `s^2` and selects `xmin`. Its CSN bootstrap resamples empirical values less than `xmin`. It samples synthetic Pareto values more than `xmin` and fits each sample. It reports `p>=0.1` as fit compatibility. This goodness-of-fit probability is distinct from the observed KS distance and is exposed separately.
...
Aspect ratio is stored as `Q=max(shape)/min(shape)`, while the MP law internally uses `1/Q`. Raw eigenvalues are usually `s^2` with no covariance denominator. Some archive cells normalize a Gram matrix by trace. Other cells use one dimension. Thus, notebook plots do not all use one scale.

The simplest MP scale estimate removes a selected number of large eigenvalues. It uses the largest retained value as the MP upper edge. Then it solves for `sigma`. The automated archive fitter builds a linear-kernel KDE over eigenvalues and minimizes the residual between that KDE and the MP density while floating only `sigma`. The maintained `kde_bulk_fit` keeps its trimming and bandwidth interface. It minimizes an integrated ECDF objective with a bounded SciPy optimizer. The objective uses the full sample and includes the zero-mass rank offset. Thus, it does not compare unsmoothed density at the square MP hard edge.

The archive defines stable rank as `sum(lambda)/max(lambda)` and MP soft rank as `lambda_plus/lambda_max`. It also defines hard rank, tensor reshaping, localization ratios, and spike locations. Its matrix entropy first divides a rectangular matrix by `trace(W)`. Many shapes do not have this value. The later singular-energy probabilities do not depend on scale. Phase I squared-singular-value entropy is retained, and `normalized_matrix_entropy` reproduces the archive final division by `log(numerical_rank)` without the invalid trace step.

Power-law analysis delegates `xmin` selection and distribution comparison to the external `powerlaw` package. There is no fixed-window MLE implementation in the supplied archive. The requested `fixed_cutoff_mle` is thus implemented as a new compatibility method, not described as codebase3 source behavior.
...
Abi Younes, Ding, and Trogdon consider a positive sample covariance matrix with a one-interval asymptotic density having square-root edges and finitely many separated right outliers. Lanczos at a random unit probe returns the finite Jacobi matrix of the vector empirical spectral distribution. The Jacobi matrix is Cholesky-factorized. Under the paper assumptions, the diagonal and subdiagonal Cholesky entries converge exponentially to constants `alpha` and `beta`, giving support estimates

`gamma_minus = (alpha-beta)^2` and `gamma_plus = (alpha+beta)^2`.

The constant tail has an analytic Stieltjes transform. Earlier finite entries are folded back by the paper continued-fraction recurrence. Multiple probes are combined by averaging sufficiently deep Cholesky entries before constructing each transform. Spikes are poles to the right of `gamma_plus + C N^(-delta)`, with `0<delta<1/2`. The paper numerical section uses `C=1` and `delta=0.25` and notes that this tuning remains consequential.

Implementation decision: `lanczos_stieltjes.py` uses adaptive Lanczos with double reorthogonalization. It uses reference-order tail changes, Cholesky factorization, consensus support, and per-probe continued fractions. One public measure gives representative poles and transforms. A separate transform averages probe density. Finite VEST gives Ritz poles. The Phase II extended constant-tail section remains an explicit alternative. It accepts a dense symmetric matrix or a matrix-free covariance operator constructed from a rectangular factor. Results state convergence and assumption diagnostics. The method is asymptotically consistent under the paper conditions. It is not described as exact or fluctuation-free for arbitrary neural weight spectra.

## 7. CLI reconciliation and dispatch decisions

| CLI option | Runtime method | Provenance and caveat |
|---|---|---|
| `--mp-fit-method analytic_mp` | Resistant MP quantile scale fit | Phase I corrected baseline |
...
| `--mp-fit-method kde_bulk_fit` | Upper-trimmed, full-sample ECDF fit of MP scale | Hardened descendant of codebase3 automated KDE notebook. Finite at the square hard edge |
...
- `src/rmt_utils.py`: all reusable broadening, MP, spacing, number-variance, Brody, PT, IPR, Hill, and CSN methods described in section 5.2.
...
- `WeightWatchers.ipynb`, `WeightWatcherTest.ipynb`: exploratory aggregate model diagnostics and tests.
...
- `Keras-Save-Load-Test.ipynb`: serialization test.
...
- `allennlp/run_analyzers.sh`: shell orchestration for the two AllenNLP reports. It adds no numerical method and was not operated.
...
Synthetic tests can examine formulas, dispatch, deterministic sampling, support estimates, and pure-module isolation. They cannot specify trained-layer exponents or promise a FARMS improvement. They also cannot specify the relative effects of bottom and equal-energy bulk lesions. Those remain empirical report hypotheses. The source tree thus separates numerical calibration assertions from claims that require training and benchmarking by an operator.
